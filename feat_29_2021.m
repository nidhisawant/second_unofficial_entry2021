function data = feat_29_2021(ecg,QRS,fs)
%
...
%% Class determination Normal(N)/AF(A)/Others(O)/Noise(~)
 ...
 %Reading 
ecg300=resample(ecg,300,fs);
%ecg=(ecg-nanmean(ecg))./nanstd(ecg);
if size(ecg300,2)<10*300
dd=[ecg300 ecg300];
dd2=[ecg ecg];
ecg=dd2(1,1:10*fs);
ecg300= dd(1,1:10*300);
QRS_10sec=QRS;
elseif size(ecg300,2)>10*300
    ecg=ecg(1,1:10*fs);
    ecg300=ecg300(1,1:10*300);
    qrs_idx=find(QRS>(10*fs));
    if isempty(qrs_idx)
        QRS_10sec=QRS;
    else
    QRS_10sec=QRS(1:qrs_idx(1)-1);
    end
else
    QRS_10sec=QRS;
end
...
%try
 ...
 if length(QRS)<6
    ...
    SPEe=mean(pentropy(ecg,fs));%spectral entropy
    SHEe=wentropy(ecg,'shannon')/length(ecg);%shanon entropy%---------
    %--------Teager energy----
    TeagerHR=sum(Teager(HR).^2);
    TKurtoHR=kurtosis(Teager(HR));
    TKurtoecg=kurtosis(Teager(ecg300));
   ...
   SPE=mean(pentropy(HR,1));
    ...
    [ segs ] = segment_ecg_RtoR( ecg300,QRS_10sec,fs );
    data21 = morphoroughness(segs );
    data2=[ data21([2 4])];
    
    datai= [data1 data2 ];
    datam= morpho_ecg2( ecg300,QRS_10sec,fs);
    
    datam2= morpho_ecg3( ecg300,QRS_10sec,fs );

%           Snpecg=sampen(ecg(750:end),30,0.1,[],0,[]);
           Snpecg=sampen(ecg300,30,0.1,[],0,[]); 
...
QRS1=floor((QRS/fs).*300);
for i=1:(length(QRS1)-2)
    
 if QRS1(1,i)>20 
     
[aa bb cc dd]= (findpeaks((ecg((QRS1(1,i+1)-(0.07*300)):(QRS1(1,i+1)+(0.17*300)))),'Annotate','extents','SortStr','descend'));


[aa1 bb1 cc1 dd1] = findpeaks((-ecg((QRS1(1,i+1)-(0.07*300)):(QRS1(1,i+1)+(0.17*300)))),'Annotate', 'extents', 'SortStr','descend');
